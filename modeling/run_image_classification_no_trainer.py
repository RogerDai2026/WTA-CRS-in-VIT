...
import transformers
from transformers import AutoConfig, AutoImageProcessor, SchedulerType, get_scheduler
...
def main():
    ...
    #approx_config1 = json.loads(args.approx_config) if args.approx_config is not None else None
    
    from approxlinear import ApproxLinearConfig
    ...
    # for step,  batch in enumerate(train_dataloader._get_iterator()):
    #     print(step)
    #     ipdb.set_trace()
    #     break
    ...
    if Scheme.scheme_open:
        Scheme.num_samples = train_dataset.num_rows
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, (index, input) in enumerate(active_dataloader):
            #ipdb.set_trace()
            if Scheme.scheme_open:#if Scheme.scheme_open:
                Scheme.batch = index
                if epoch == 0 and step == 0:
                    with torch.no_grad():
                        model.train()
                        input = {k: v.to('cuda') for k, v in input.items()} if isinstance(input, dict) else input.to('cuda')
                        #ipdb.set_trace() 
                        outputs = model(**input)
                    Scheme._init_buffer()
                Scheme.fetch_data()
        ...
        initial_mem_usage = torch.cuda.memory_allocated() # Initial memory usage
        peak_mem_usage = initial_mem_usage  # To track the peak memory usage
        for step, (index, batch) in enumerate(eval_dataloader):
            with torch.no_grad():
                outputs = model(**batch)
            ...
            current_mem_usage = torch.cuda.max_memory_reserved()
            if current_mem_usage > peak_mem_usage:
                peak_mem_usage = current_mem_usage

        eval_metric = metric.compute()
        
        logger.info(f'Epoch {epoch}: Accuracy={eval_metric}, Loss={total_loss.item() / len(train_dataloader)}, peak Memory Usage={peak_mem_usage/1e9 } GB')
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            all_results['dataset_name'] = args.dataset_name
            all_results['epoch'] = args.num_train_epochs  
            all_results['peak_memory_usage'] = peak_mem_usage/1e9
