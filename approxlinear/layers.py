...
from .scheme import Scheme_3D_new, Scheme_4D_new, Scheme_3D, Scheme_4D
...
class ApproxLinear(torch.nn.Linear):
    def __init__(self, input_features, output_features, bias=True, config=None, batch_dim_use_same_indices=False):
        ...
        # self.scheme = Scheme_3D(config.sampling_ratio, config.deter_ratio, config.deter_adaptive, config.minimal_k,
                                # config.sample_replacement, config.mix_replacement, self.batch_dim_use_same_indices)
    ...
    def forward(self, input):
        ...
        if input.dim() != 3:
            print("Input tensor must have 3 dimensions, but got {}".format(input.dim()))
        if not self.scheme.inited and self.weight.requires_grad:
