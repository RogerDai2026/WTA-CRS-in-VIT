...
@torch.no_grad()
def subsample_4D_B_by_norm(A, B, weight, scheme, eps=1e-8):
    # A: b x h x n
    # B: b x h x n x d
    # print(f'A: {A.shape}')
    # print(f'B: {B.shape}')
    ...
    prob = (norm_mult + eps) / (norm_mult.sum(dim=1).unsqueeze(dim=1) + eps)
...
def sample_with_replacement_4D(B, prob, stoc_k, shape, top_k_prob, a_sum, longtail):
    ...
    # if torch.any((prob_ > 0.).sum(dim=1) == 0):
    #     import pdb
    #     pdb.set_trace()
...
@torch.no_grad()
def subsample_3D_input_by_norm(input, grad_norm, scheme, eps=1e-8):
    """
    ...
    prob = (norm_mult + eps) / (norm_mult.sum() + eps)
